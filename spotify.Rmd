...
```{r}
   ...
   fav_tracks_unique <- fav_tracks %>%
      distinct (track.id) %>%
      pull (track.id)
   
   fav_tracks_audio_features <-get_track_audio_features (fav_tracks_unique) %>%
      right_join(fav_tracks, by = c("id" = "track.id")) %>%
      select (track.artists, track.name, track.popularity, track.album.release_date, danceability:tempo)
```
...
```{r transform, message = F, results = F, warning = F}
   ...
   songs_audio_features <- get_track_audio_features (recent_tracks) %>%
      left_join(songs, by = c("id" = "track_id")) %>%
      select (artist_name: release, danceability:tempo)
```


### 4.  Favorite tracks by artist count
```{r}
