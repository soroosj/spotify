...
```{r warning = TRUE, results = FALSE, message = FALSE}
   ...
   favorite_tracks_features <- 
      get_track_audio_features(favorite_tracks_unique[1:100]) %>%
      bind_rows(get_track_audio_features (favorite_tracks_unique[101:200])) %>%
...
```{r warning = TRUE, results = FALSE, message = FALSE}
   ...
   #favorite_tracks_features <- get_track_audio_features(favorite_tracks_unique[1:100]) %>%
      #bind_rows(get_track_audio_features (favorite_tracks_unique[101:200]))
   ...
   #favorite_tracks_features <- 
      ...
      #seq(1) %>%
      #map(function(x) {
          #get_track_audio_features (favorite_tracks_unique[x:x+99])
            #}) %>%
      #unnest()
   
   #favorite_tracks_features <- favorite_tracks_features %>%
      #right_join(favorite_tracks, by = c("id" = "track.id")) %>%
      #rename (artist.name = name) %>%
     # mutate (
         #duration = duration_ms / 1000 / 60,
