...
```{r}
   ...
   favorite_tracks_features <-get_track_audio_features (favorite_tracks_unique) %>%
      ...
      mutate (
         ...
         release = year (as_date(track.album.release_date))
         ) %>%
      select (artist.name, track.name, danceability, energy, valence, tempo, release, duration) %>%
   ...
   #function to create bar chart for dynamic variable
      attributes_plot = function(attribute = "tempo") {
         ...
         ggplot (
            ...
            #geom_hline(
               #data = favorite_tracks_features %>% summarize (med = median(!!attribute)),
               #aes(yintercept = med), 
               #alpha = 0.2, size = 0.75
               #) +
            geom_rug (
            ...
            coord_flip () +
            theme(
               plot.title = element_text(hjust = 0.5, size = 10, face = "bold", color = "black", margin = margin (10,0,10,0)),
...
```{r warning = TRUE, results = FALSE, message = FALSE}
   ...
   #combine charts into grid
      song_plots_combine <- 
         ...
         plot_annotation (
            title =  "Favorite songs on Spotify",
            subtitle = "Top 5 attributes in green, bottom 5 in red",
            caption = "Visualization: Joel Soroos @soroosj\n Data: Spotify API",
            theme =
               theme (
                  plot.title = element_text(hjust = 0.5, vjust = 0, size = 15, face = "bold", margin = margin (0,0,5,0)),
                  plot.title.position = "plot",
                  plot.subtitle = element_text(hjust = 0.5, vjust = 0, size = 10, margin = margin (0,0,15,0)),
